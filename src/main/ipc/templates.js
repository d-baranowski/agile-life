...
import { getTemplateGroups, createTemplateGroup, updateTemplateGroup, deleteTemplateGroup, getTemplatesByGroup, createTicketTemplate, updateTicketTemplate, deleteTicketTemplate, getBoardById, upsertCards } from '../database/db';
import { TrelloClient } from '../trello/client';
/** Supported mustache-style placeholders and their resolved values at call time. */
function resolvePlaceholders(template, now) {
...
export function registerTemplateHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_GET_GROUPS, async (_e, boardId) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_CREATE_GROUP, async (_e, boardId, input) => {
        try {
            if (!input.name?.trim()) {
    ...
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_UPDATE_GROUP, async (_e, boardId, id, input) => {
        try {
            if (!input.name?.trim()) {
                return { success: false, error: 'Group name is required.' };
            }
    ...
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_DELETE_GROUP, async (_e, boardId, id) => {
        try {
    ...
    // ── TEMPLATES_GET ──────────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_GET, async (_e, boardId, groupId) => {
        try {
    ...
    // ── TEMPLATES_CREATE ───────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_CREATE, async (_e, boardId, input) => {
        try {
    ...
    // ── TEMPLATES_UPDATE ───────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_UPDATE, async (_e, boardId, id, input) => {
        try {
            if (!input.titleTemplate?.trim()) {
                return { success: false, error: 'Title template is required.' };
            }
    ...
    // ── TEMPLATES_DELETE ───────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_DELETE, async (_e, boardId, id) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.TEMPLATES_GENERATE_CARDS, async (_e, boardId, groupId) => {
        try {
