...
import { getAllBoards, addBoard, updateBoard, deleteBoard, getBoardById, upsertLists, markRemovedLists, upsertCards, markRemovedCards, updateBoardSyncTime, getListsForBoard, getCardsForBoard, moveCardToList, updateCardPos, archiveCardLocally, updateCardMembers, upsertBoardMembers, getBoardMembers, getCardMembersJson, upsertActions, getLatestActionDate, upsertCardListEntry, setCardListEntryFallback, isCardListEntriesInitialized, setCardListEntriesInitialized, clearCardListEntriesForBoard, getDoneCardsOlderThan, getDoneColumnDebug, getDb } from '../database/db';
import { TrelloClient } from '../trello/client';
import sqlColumnCounts from '../database/sql/analytics/column-counts.sql?raw';
export function registerBoardHandlers() {
    // ── Board CRUD ──────────────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.BOARDS_GET_ALL, async () => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.BOARDS_ADD, async (_e, input) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.BOARDS_UPDATE, async (_e, boardId, updates) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.BOARDS_DELETE, async (_e, boardId) => {
        try {
            deleteBoard(boardId);
            return { success: true };
        }
    ...
    ipcMain.handle(IPC_CHANNELS.BOARDS_FETCH_FROM_TRELLO, async (_e, apiKey, apiToken) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_SYNC, async (_e, boardId) => {
        try {
            ...
            markRemovedLists(boardId, freshLists.map((l) => l.id));
            upsertCards(boardId, freshCards);
            markRemovedCards(boardId, freshCards.map((c) => c.id));
    ...
    ipcMain.handle(IPC_CHANNELS.ANALYTICS_COLUMN_COUNTS, async (_e, boardId) => {
        try {
            const rows = getDb().prepare(sqlColumnCounts).all(boardId);
    ...
    // Always reads from the local SQLite cache — call TRELLO_SYNC first.
    ipcMain.handle(IPC_CHANNELS.TRELLO_GET_BOARD_DATA, async (_e, boardId) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_MOVE_CARD, async (_e, boardId, cardId, toListId, pos) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_UPDATE_CARD_POS, async (_e, boardId, cardId, pos) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_PREVIEW_ARCHIVE_DONE_CARDS, async (_e, boardId, olderThanWeeks) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
            const cutoffDate = new Date(Date.now() - olderThanWeeks * 7 * 24 * 60 * 60 * 1000).toISOString();
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_GET_DONE_COLUMN_DEBUG, async (_e, boardId) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_ARCHIVE_DONE_CARDS, async (_e, boardId, olderThanWeeks) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_ARCHIVE_CARD, async (_e, boardId, cardId) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_GET_BOARD_MEMBERS, async (_e, boardId) => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.TRELLO_ASSIGN_CARD_MEMBER, async (_e, boardId, cardId, memberId, assign) => {
        try {
            const config = getBoardById(boardId);
            if (!config)
                return { success: false, error: `Board not found: ${boardId}` };
            ...
            const currentMembers = JSON.parse(membersJson);
