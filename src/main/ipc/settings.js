...
export function registerSettingsHandlers() {
    // ── Get DB path info ────────────────────────────────────────────────────────
    ipcMain.handle(IPC_CHANNELS.SETTINGS_GET_DB_PATH, async () => {
        try {
    ...
    ipcMain.handle(IPC_CHANNELS.SETTINGS_SET_DB_PATH, async (_e, resetToDefault) => {
        try {
            if (resetToDefault) {
                setDbPath(null);
            }
                ...
                if (fs.existsSync(currentPath)) {
                    ...
                    console.warn('[settings] Current database file not found at', currentPath, '— new location will start with an empty database.');
                }
                setDbPath(filePath);
            }
            const defaultPath = getDefaultDbPath();
