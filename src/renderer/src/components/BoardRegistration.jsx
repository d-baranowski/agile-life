...
function getTrelloAuthorizeUrl(apiKey) {
...
export default function BoardRegistration({ onBoardAdded, onCancel }) {
    ...
    const handleFetchBoards = async () => {
        ...
        if (looksLikeUrl(normalizedApiKey)) {
            setError('That looks like a Trello settings page URL, not the API key itself. Open the page and copy the API Key value.');
            return;
        }
        setLoading(true);
        setError(null);
        ...
        if (!result.success || !result.data) {
            setError(result.error === 'Invalid Trello API credentials'
    ...
    return (<div className={styles.container}>
      <div className={styles.card}>
        ...
        <div className={styles.header}>
          ...
          {onCancel && (<button className="btn-ghost" onClick={onCancel}>
              ✕ Cancel
            </button>)}
        </div>
        ...
        <div className={styles.steps}>
          {['credentials', 'select-board', 'configure'].map((s, idx) => (<div key={s} className={`${styles.step} ${step === s ? styles.stepActive : ''} ${['credentials', 'select-board', 'configure'].indexOf(step) > idx
                ? styles.stepDone
                : ''}`}>
        ...
        {/* ── Step 1: Credentials ── */}
        {step === 'credentials' && (<div className={styles.form}>
            <div className={styles.infoPanel}>
              ...
              <div className={styles.linkRow}>
                <a className={styles.actionLink} href="https://trello.com/app-key" target="_blank" rel="noreferrer">
                  Open Trello API key page
                </a>
                <a className={`${styles.actionLink} ${!normalizedApiKey ? styles.actionLinkDisabled : ''}`} href={normalizedApiKey ? authorizeUrl : undefined} target="_blank" rel="noreferrer" aria-disabled={!normalizedApiKey} onClick={(event) => {
                if (!normalizedApiKey) {
            ...
            <label className={styles.label}>
              Trello API Key
              <input type="text" placeholder="Paste the API key value" value={apiKey} onChange={(e) => setApiKey(e.target.value)} autoComplete="off"/>
            </label>
            <label className={styles.label}>
              Trello API Token
              <input type="password" placeholder="Paste the token shown after browser approval" value={apiToken} onChange={(e) => setApiToken(e.target.value)} autoComplete="off"/>
            </label>
        ...
        {/* ── Step 2: Select Board ── */}
        {step === 'select-board' && (<div className={styles.form}>
            <p className={styles.hint}>
              {boards.length} board{boards.length !== 1 ? 's' : ''} found. Select the one you want
              to register.
            </p>
            <div className={styles.boardList}>
              {boards.map((board) => (<button key={board.id} className={`${styles.boardOption} ${selectedBoardId === board.id ? styles.boardOptionSelected : ''}`} onClick={() => setSelectedBoardId(board.id)}>
            ...
            <div className={styles.actions}>
              ...
              <button className="btn-primary" onClick={handleSelectBoard} disabled={!selectedBoardId}>
                Next →
              </button>
            </div>
          </div>)}
        ...
        {/* ── Step 3: Configure ── */}
        {step === 'configure' && (<div className={styles.form}>
            <p className={styles.hint}>
            ...
            <label className={styles.label}>
              ...
              <input type="text" placeholder="AGI" maxLength={3} value={projectCode} onChange={(e) => setProjectCode(e.target.value.toUpperCase())}/>
            </label>
            <label className={styles.label}>
              &quot;Done&quot; List Names{' '}
              <span className={styles.optional}>(comma-separated, e.g. Done, Shipped)</span>
              <input type="text" placeholder="Done" value={doneListNames} onChange={(e) => setDoneListNames(e.target.value)}/>
            </label>
            <div className={styles.actions}>
