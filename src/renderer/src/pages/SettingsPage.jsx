...
export default function SettingsPage({ board, onBoardUpdated, onBoardDeleted }) {
    ...
    const [storyPoints, setStoryPoints] = useState(board.storyPointsConfig);
    const [spSaving, setSpSaving] = useState(false);
    const [spSuccess, setSpSuccess] = useState(null);
    ...
    const [debugCards, setDebugCards] = useState(null);
    ...
    const [dbPathInfo, setDbPathInfo] = useState(null);
    ...
    return (<div className={styles.container}>
      ...
      <div className="card">
        ...
        <div className={styles.form}>
          ...
          <label className={styles.label}>
            ...
            <input type="text" value={doneListNames} onChange={(e) => setDoneListNames(e.target.value)} placeholder="Done, Shipped, Closed"/>
          </label>
        </div>
      ...
      <div className="card">
        ...
        <table className={styles.spTable}>
          ...
          <tbody>
            {storyPoints.map((rule, idx) => (<tr key={idx}>
                <td>
                  <input type="text" value={rule.labelName} placeholder="e.g. Large" onChange={(e) => {
                ...
                <td>
                  <input type="number" min={0} value={rule.points} onChange={(e) => {
                const next = [...storyPoints];
                next[idx] = {
                    ...next[idx],
                ...
                <td>
                  <button className="btn-ghost" onClick={() => setStoryPoints(storyPoints.filter((_, i) => i !== idx))} title="Remove rule">
                    ✕
                  </button>
                </td>
              </tr>))}
        ...
        <div className={styles.spActions}>
          <button className="btn-ghost" onClick={() => setStoryPoints([...storyPoints, { labelName: '', points: 1 }])}>
            + Add Rule
          </button>
      ...
      <div className="card">
        ...
        <div className={styles.archiveControls}>
          <label className={styles.weeksLabel}>
            ...
            <input type="number" min={1} max={52} value={archiveWeeks} onChange={(e) => setArchiveWeeks(Math.max(1, parseInt(e.target.value, 10) || 1))} className={styles.weeksInput}/>
            week{archiveWeeks !== 1 ? 's' : ''}
          </label>
          <button className="btn-secondary" onClick={handlePreview} disabled={previewing || archiving}>
            {previewing ? (<span className={styles.syncingLabel}>
        ...
        {archiveResult && (<div className={styles.archiveSuccess}>
        ...
        {previewCards !== null && (<div className={styles.previewSection}>
            {previewCards.length === 0 ? (<p className={styles.previewEmpty}>
                ...
                <ul className={styles.previewList}>
                  {previewCards.map((card) => (<li key={card.id} className={styles.previewItem}>
                ...
                <div className={styles.previewActions}>
                  <button className="btn-danger" onClick={handleArchive} disabled={archiving}>
                    {archiving ? (<span className={styles.syncingLabel}>
                        <span className="spinner" style={{ width: 14, height: 14, borderWidth: 2 }}/>
                        Archiving…
                      </span>) : (`🗄 Archive ${previewCards.length} Card${previewCards.length !== 1 ? 's' : ''}`)}
                  </button>
                  <button className="btn-secondary" onClick={() => setPreviewCards(null)} disabled={archiving}>
                    Cancel
                  </button>
                </div>
              </>)}
      ...
      <div className="card">
        <div className={styles.debugHeader}>
          ...
          <button className="btn-ghost" onClick={handleDebugToggle} disabled={debugLoading} style={{ flexShrink: 0 }}>
            {debugOpen ? '▲ Hide' : '▼ Show'}
          </button>
        </div>
        ...
        {debugOpen && (<>
            ...
            {debugLoading && (<div className={styles.centred}>
                <div className="spinner"/>
                <span>Loading…</span>
              </div>)}
            {!debugLoading && debugCards !== null && (<>
                {debugCards.length === 0 ? (<p className={styles.previewEmpty}>
                  ...
                  </p>) : (<div className={styles.debugTableWrap}>
                    <table className={styles.debugTable}>
                      ...
                      <tbody>
                        {debugCards.map((card) => (<tr key={card.id}>
                            ...
                            <td>
                              <span className={card.hasActionEntry
      ...
      <div className="card">
        ...
        {dbPathChanged && (<div className={styles.successBanner}>
        ...
        <div className={styles.form}>
          <label className={styles.label}>
            ...
            <input type="text" readOnly value={dbPathInfo?.currentPath ?? '…'} style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}/>
          </label>
        </div>
        <div className={styles.actions}>
          {dbPathInfo?.isCustom && (<button className="btn-ghost" onClick={handleResetDbPath} disabled={dbPathChanging}>
              Restore Default
            </button>)}
      ...
      <div className={`card ${styles.dangerCard}`}>
        ...
        {confirmDelete ? (<div className={styles.confirmDelete}>
          ...
          </div>) : (<button className="btn-danger" onClick={() => setConfirmDelete(true)}>
            Remove Board
          </button>)}
      </div>
    </div>);
