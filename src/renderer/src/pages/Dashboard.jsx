...
export default function Dashboard({ board, syncVersion }) {
    const [columns, setColumns] = useState([]);
    const [weeklyStats, setWeeklyStats] = useState([]);
    const [labelStats, setLabelStats] = useState([]);
    const [historyStats, setHistoryStats] = useState([]);
    const [storyPointStats, setStoryPointStats] = useState([]);
    ...
    const loadAll = useCallback(async () => {
        ...
        else if (!colResult.success)
            setError(colResult.error ?? 'Failed to load column counts.');
        if (weeklyResult.success)
            setWeeklyStats(weeklyResult.data ?? []);
        else
            setError((prev) => prev ?? weeklyResult.error ?? 'Failed to load weekly stats.');
        ...
        if (spResult.success)
            setStoryPointStats(spResult.data ?? []);
        setLoading(false);
    ...
    const userTotals = weeklyStats.reduce((acc, row) => {
    ...
    const labelGroups = labelStats.reduce((acc, row) => {
    ...
    historyStats.forEach((r) => {
        const key = r.userId ?? 'unassigned';
        if (!historyLookup.has(r.week))
            historyLookup.set(r.week, new Map());
    ...
    const pageRangeLabel = pageWeeks.length > 0
    ...
    return (<div className={styles.container}>
      ...
      <div className={styles.header}>
        <div>
          ...
          {board.lastSyncedAt && (<span className={styles.lastSync}>
      ...
      {loading ? (<div className={styles.centred}>
          <div className="spinner"/>
          <span>Loading…</span>
        </div>) : (<>
          {/* ── Combined summary strip ── */}
          {(columns.length > 0 || sortedUsers.length > 0) && (<div className={styles.statsStrip}>
          ...
          {/* ── Cards per Column ── */}
          {columns.length === 0 ? (<div className={styles.emptyState}>
              <p>No data yet.</p>
              ...
              <div className={styles.columnsGrid}>
                {columns.map((col) => {
                    ...
                    return (<div key={col.listId} className={styles.columnCard}>
          ...
          {sortedUsers.length === 0 && columns.length > 0 && (<p className={`text-muted ${styles.analyticsHint}`}>
          ...
          {sortedUsers.length > 0 && (<section>
              ...
              <div className={styles.userList}>
                {sortedUsers.map(([userId, { userName, count }]) => {
                    ...
                    return (<div key={userId} className={styles.userRow}>
          ...
          {storyPointStats.length > 0 && (<section>
              ...
              <div className={styles.userList}>
                {storyPointStats.map((row) => {
                    ...
                    return (<div key={row.userId ?? 'unassigned'} className={styles.userRow}>
          ...
          <section>
            <div className={styles.chartHeader}>
              ...
              {allHistoryWeeks.length > 0 && (<div className={styles.chartNav}>
                  <button className={styles.chartNavBtn} onClick={() => setHistoryOffset((o) => Math.min(o + 1, maxOffset))} disabled={clampedOffset >= maxOffset} title="View older weeks">
                    ◀
                  </button>
                  <span className={styles.chartNavLabel}>{pageRangeLabel}</span>
                  <button className={styles.chartNavBtn} onClick={() => setHistoryOffset((o) => Math.max(o - 1, 0))} disabled={clampedOffset === 0} title="View newer weeks">
                    ▶
                  </button>
                </div>)}
            </div>
            {allHistoryWeeks.length === 0 ? (<p className="text-muted">
                {columns.length > 0 ? (<>
              ...
              </p>) : (<div className={styles.chartWrap}>
                <Line data={historyChartData} options={historyChartOptions}/>
              </div>)}
          </section>

          {/* ── Tickets by Label ── */}
          {Object.keys(labelGroups).length > 0 && (<section>
              <h2 className={styles.sectionTitle}>Tickets by Label</h2>
              <div className={styles.labelList}>
                {Object.entries(labelGroups).map(([labelName, { color, users }]) => (<div key={labelName} className={styles.labelGroup}>
                    <div className={styles.labelHeader}>
                      <span className={styles.labelDot} style={{ background: resolveLabelColor(color) }}/>
                    ...
                    <table>
                      ...
                      <tbody>
                        {users.map((u) => (<tr key={u.userId ?? 'unassigned'}>
                            <td>{u.userName}</td>
