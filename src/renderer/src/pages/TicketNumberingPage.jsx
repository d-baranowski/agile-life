...
export default function TicketNumberingPage({ board }) {
    ...
    const skipSetRef = useRef(new Set());
    ...
    const removeFromPreview = (cardId) => {
    ...
    const handleApply = async () => {
        ...
        const initial = preview.map((card) => ({
            card,
            status: 'queued',
            showError: false
        }));
        ...
        for (let i = 0; i < initial.length; i++) {
            ...
            if (skipSetRef.current.has(cardId))
                continue;
            ...
            if (result.success) {
                ...
                setCardStates((prev) => prev
                    ? prev.map((cs) => cs.card.cardId === cardId ? { ...cs, status: 'error', error: result.error } : cs)
                    : prev);
            }
        }
    ...
    const toggleErrorDetail = (cardId) => {
        setCardStates((prev) => prev
    ...
    const summaryStyle = !applying && errorCount === 0 && cancelledCount === 0
    ...
    return (<div className={styles.container}>
      ...
      <div className="card">
        ...
        <div className={styles.configGrid}>
          <label className={styles.label}>
            ...
            <input className="input" value={projectCode} maxLength={3} placeholder="AGI" onChange={(e) => setProjectCode(e.target.value.toUpperCase())}/>
          </label>

          <label className={styles.label}>
            Next Ticket Number
            <span className={styles.hint}>Never lower than the current value</span>
            <input className="input" type="number" min={config?.nextTicketNumber ?? 1} value={nextTicketNumber} onChange={(e) => setNextTicketNumber(Number(e.target.value))}/>
          </label>
        </div>
      ...
      {config && (<div className="card">
          ...
          <div className={styles.statusRow}>
            <span className={`${styles.statusBadge} ${config.unnumberedCount === 0 ? styles.statusOk : styles.statusWarn}`}>
      ...
      <div className="card">
        ...
        {cardStates && (<div className={`${styles.progressSummary} ${summaryStyle}`}>
            {applying ? (<>
        ...
        {/* Empty state */}
        {preview && preview.length === 0 && !cardStates && (<p className={styles.description}>No unnumbered cards found — nothing to do.</p>)}

        {/* Card table — preview mode (before apply) */}
        {preview && preview.length > 0 && !cardStates && (<table className={styles.table}>
            <thead>
              <tr>
                <th>List</th>
                <th>Current Name</th>
            ...
            <tbody>
              {preview.map((card) => (<tr key={card.cardId}>
                  ...
                  <td>
                    <button className={styles.removeBtn} title="Remove from queue" onClick={() => removeFromPreview(card.cardId)}>
                      ✕ Remove
                    </button>
                  </td>
                </tr>))}
        ...
        {cardStates && (<table className={styles.table}>
            <thead>
              <tr>
            ...
            <tbody>
              {cardStates.map((cs) => (<>
                  <tr key={cs.card.cardId} className={`${styles.cardRow} ${styles[`row-${cs.status}`]}`}>
                    <td className={styles.statusCell}>
                      {cs.status === 'queued' && (<span className={styles.badgeQueued}>⏳ Queued</span>)}
                      {cs.status === 'in-progress' && (<span className={styles.badgeInProgress}>
                          <span className={styles.spinner}/> Updating…
                        </span>)}
                      {cs.status === 'success' && (<span className={styles.badgeSuccess}>✅ Done</span>)}
                      {cs.status === 'cancelled' && (<span className={styles.badgeCancelled}>🚫 Cancelled</span>)}
                      {cs.status === 'error' && (<span className={styles.badgeError}>
                          ❌ Failed
                          <button className={styles.errorToggle} onClick={() => toggleErrorDetail(cs.card.cardId)}>
                    ...
                    <td>
                      {cs.status === 'queued' && (<button className={styles.removeBtn} title="Remove from queue" onClick={() => removeFromQueue(cs.card.cardId)}>
                          ✕ Remove
                        </button>)}
                    </td>
                  </tr>
                  {cs.status === 'error' && cs.showError && cs.error && (<tr key={`${cs.card.cardId}-err`} className={styles.errorDetailRow}>
                      <td colSpan={5}>
        ...
        <div className={styles.actions}>
          ...
          {preview && preview.length > 0 && !cardStates && (<button className="btn-primary" onClick={handleApply} disabled={applying}>
              ▶ Apply to {preview.length} card{preview.length !== 1 ? 's' : ''}
            </button>)}
          {applying && (<button className="btn-danger" onClick={handleCancel}>
              ✕ Cancel
            </button>)}
        </div>
      </div>
